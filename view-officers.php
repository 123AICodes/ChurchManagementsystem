...
<div class="main-dash-container">
  ...
  <div class="dashboard-container">
    <h1 class="title text-black">All Officers Data</h1>
    ...
    <div class="content-box-wrapper" style="display: block;">
      <div class="views-button-container">
        ...
        <button class="views-btn active" onclick="window.location.href='<?= ROOT_URL ?>view-officers.php';">Officers</button>
    ...
    <div style="margin-top: 2rem;" class="sub-container">
      ...
      <div class="wrapper">
        ...
        <div class="ellipse-dropdown el-first">
          <button class="top-btn">
            <h4 class="text-blue">Officers Recorded</h4>
          </button>
          <div class="action">
            ...
            <div class="action-box">
              ...
              <button class="action-btn" onclick="window.location.href='<?= ROOT_URL ?>reports/officers-report.php';">
        ...
        <?php
        $sno = 1;
        $fetch_members_query = " SELECT * FROM members WHERE member_type='Officer' ORDER BY id DESC";
      ...
      <div class="wrapper">
        ...
        <div class="ellipse-dropdown">
          ...
          <div class="action">
            ...
            <div class="action-box yth-bx">
              <button class="action-btn" onclick="window.location.href='<?= ROOT_URL ?>views.php;">View All</button>
              <button class="action-btn" onclick="window.location.href='<?= ROOT_URL ?>member.php';">Add Officer</button>
              <button class="action-btn border-top close-action-box" onclick="this.parentElement.remove()">close</button>
            </div>
          </div>
        </div>
        <!-- recent women box -->
        <?php
        $query = " SELECT * FROM members where member_type='Officer' ORDER BY id DESC LIMIT 5";
        ...
        <div class="recently-added-yth">
          <?php while ($member = mysqli_fetch_assoc($query_results)) : ?>
            ...
            <?php elseif ($member['title'] == 'Dcns') : ?>
              <a href="<?= ROOT_URL ?>view-women.php" class="youth d-flex">
                <div class="img-round-sm"><img src="<?= ROOT_URL ?>profile-avatar/<?= $member['profile'] ?>"></div>
                <div class="content">
                  <?php
                  require_once "./other_partials/__unixtimestamp.php";
                  ?>
                  <p class="text-gray"><?= $member['title'] . ' ' . $member['name'] ?></p>
                </div>
              </a>
